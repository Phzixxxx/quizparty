document.addEventListener('DOMContentLoaded', () => {
    console.log("✅ Script carregado!");
     // ... (todo o seu código original de perguntas permanece igual)
    ...
    // ============================================================
    // === 🌍 MODO PARTY (Multiplayer de 2 jogadores) ===
    // ============================================================
    ...
    socket.on("salaCriada", ({ codigo }) => {
      ...
      codigoDisplay.textContent = `Código da Sala: ${codigo}`;
    });

    document.getElementById("btnEntrarSala").onclick = () => {
      nomeJogador = nomeInput.value.trim();
      const codigo = codigoInput.value.trim().toUpperCase();
      if (!nomeJogador || !codigo) return alert("Preencha nome e código!");
    ...
    socket.on("partidaIniciada", () => {
      ...
      iniciarQuizCompleto(); // usa a função já existente
    });
    ...
    // === Iniciar quiz por ano específico ===
